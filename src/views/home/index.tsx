  ...
  return (
    <div className="flex flex-col h-screen w-full bg-black justify-center items-center font-mono select-none text-white overflow-hidden">
        ...
        <div className="flex items-center gap-2 rounded-full bg-white/5 px-2 py-1 mb-4 z-50 border border-white/10">
          <button onClick={() => setActiveTab('Play')} className={`rounded-full px-4 py-1 text-xs font-bold transition-all ${activeTab === 'Play' ? 'bg-slate-800 text-white shadow-lg' : 'text-slate-500'}`}>Play</button>
          <button onClick={() => setActiveTab('Rank')} className={`rounded-full px-4 py-1 text-xs font-bold transition-all ${activeTab === 'Rank' ? 'bg-slate-800 text-white shadow-lg' : 'text-slate-500'}`}>Rank</button>
          <button className="rounded-full px-3 py-1 text-slate-500 text-xs cursor-not-allowed opacity-50">Shop</button>
        </div>
        ...
        <div className="relative w-full max-w-[400px] h-full max-h-[800px] border-x-4 border-gray-900 bg-black shadow-2xl overflow-hidden rounded-3xl">
            <GameSandbox activeTab={activeTab} />
        </div>
    </div>
  );
};
...
const GameSandbox: FC<{ activeTab: string }> = ({ activeTab }) => {
  ...
  useEffect(() => {
      ...
      const savedLb = localStorage.getItem('syncOrSinkLeaderboard');
      if (savedLb) {
          setLeaderboard(JSON.parse(savedLb));
      } else {
          // Dummy data so it doesn't look empty
          const dummies = [
              { name: "Dev_Null", score: 1250, date: "12/20" },
              { name: "Satoshi", score: 980, date: "12/21" },
              { name: "Sol_Maxi", score: 450, date: "12/22" }
          ];
          setLeaderboard(dummies);
          localStorage.setItem('syncOrSinkLeaderboard', JSON.stringify(dummies));
      }
  ...
  const saveToLeaderboard = () => {
      ...
      const newEntry: LeaderboardEntry = {
          name: playerName.substring(0, 10), // Limit name length
          score: score,
      ...
      localStorage.setItem('syncOrSinkLeaderboard', JSON.stringify(newLb));
  ...
  const initWorld = () => {
    ...
    waterLevelRef.current = 600; shieldActive.current = false; shieldTimer.current = 0;
    ghostActive.current = false; ghostTimer.current = 0; lastTimeRef.current = 0;
  ...
  const spawnExplosion = (x: number, y: number, color: string, count: number = 15) => {
    for (let i = 0; i < count; i++) {
      particles.current.push({ x: x + 10, y: y + 10, vx: (Math.random() - 0.5) * 12, vy: (Math.random() - 0.5) * 12, life: 1.0, color: color, size: Math.random() * 4 + 2 });
    }
  };
  ...
  const update = (time: number) => {
    ...
    if (gameStateRef.current === 'PLAYING') {
      ...
      if (newLevel > levelRef.current) {
        ...
        if (prevEnvIdx.current !== nextEnvIdx.current) triggerEvent('level', MID, 300, '#FFF');
      }

      if (shieldActive.current) { shieldTimer.current -= deltaTime; if (shieldTimer.current <= 0) shieldActive.current = false; }
      if (ghostActive.current) { ghostTimer.current -= deltaTime; if (ghostTimer.current <= 0) { ghostActive.current = false; spawnText(MID, 300, "GHOST ENDED", '#FFF'); } }

      const activeEnv = ENVIRONMENTS[nextEnvIdx.current];
      if (Math.random() < 0.05) spawnBgProp(activeEnv.type);
      bgProps.current.forEach(p => p.y += (speedRef.current * 0.5 * p.speed) * deltaTime);
      ...
      if (frameCount.current > currentSpawnRate) {
        ...
        if (rand < 0.008 && !ghostActive.current) spawnPowerup('GHOST');
            ...
            if (blockRand < 0.35) spawnLeft(); else if (blockRand < 0.70) spawnRight(); else { const shaveGap = -(speedRef.current * 22); if (Math.random() > 0.5) { spawnLeft(0); spawnRight(shaveGap); } else { spawnRight(0); spawnLeft(shaveGap); } }
        }
      ...
      obstacles.current.forEach((obs, i) => {
        ...
        if (pHitX < obsHitX + obsHitW && pHitX + pHitW > obsHitX && pHitY < obsHitY + obsHitH && pHitY + pHitH > obsHitY) {
          if (obs.type === 'ORB') {
            ...
            obstacles.current.splice(i, 1); triggerEvent('level', pX, p.y, '#FFF'); return;
          }
          if (obs.type === 'GHOST') {
            if (!p.grounded) return;
            ghostActive.current = true; ghostTimer.current = 480; spawnText(MID, 300, "GHOST MODE!", '#d946ef');
            obstacles.current.splice(i, 1); triggerEvent('level', pX, p.y, '#d946ef'); return;
          }
            ...
            if (shieldActive.current) {
              ...
              obstacles.current.splice(i, 1); shakeRef.current = 10; triggerEvent('crash', pX, p.y, '#F00');
            } else if (p.flash <= 0) {
              shakeRef.current = 20; spawnExplosion(pX, p.y, activeEnv.accent, 30); triggerEvent('crash', pX, p.y, '#F00');
              if (!GOD_MODE) { gameStateRef.current = 'GAMEOVER'; setGameState('GAMEOVER'); waterLevelRef.current = 600; checkAchievements(scoreRef.current); if (bgmRef.current) { bgmRef.current.pause(); bgmRef.current.currentTime = 0; } } else { p.flash = 20; spawnText(MID, 300, "HIT! (GOD MODE)", '#FF0000'); }
            }
          }
        }
    ...
    if (gameStateRef.current === 'GAMEOVER' && waterLevelRef.current > 0) waterLevelRef.current -= 15 * deltaTime;
    ...
    const prevEnv = ENVIRONMENTS[prevEnvIdx.current]; const prevGrad = ctx.createLinearGradient(0, 0, 0, H);
    prevGrad.addColorStop(0, prevEnv.bgTop); prevGrad.addColorStop(1, prevEnv.bgBot);
    ctx.fillStyle = prevGrad; ctx.fillRect(-1, -1, W + 2, H + 2); 

    if (transitionProgress.current > 0) {
        const nextEnv = ENVIRONMENTS[nextEnvIdx.current]; const nextGrad = ctx.createLinearGradient(0, 0, 0, H);
        nextGrad.addColorStop(0, nextEnv.bgTop); nextGrad.addColorStop(1, nextEnv.bgBot);
    ...
    obstacles.current.forEach(obs => {
      ...
      else { ctx.shadowBlur = 15; const color = obs.lane === 'LEFT' ? ENVIRONMENTS[nextEnvIdx.current].accent : '#FFF'; ctx.shadowColor = color; ctx.fillStyle = color; ctx.fillRect(obs.x, obs.y, obs.w, obs.h); ctx.fillStyle = '#000'; ctx.fillRect(obs.x + 2, obs.y + 2, obs.w - 4, obs.h - 4); }
    });
    ...
    const drawPlayer = (p: Player, xOffset: number, color: string) => {
      ...
      ctx.fillStyle = color; if (shieldActive.current) { ctx.strokeStyle = '#fff'; ctx.lineWidth = 2; ctx.beginPath(); ctx.arc(x + PLAYER_SIZE/2, p.y + PLAYER_SIZE/2, PLAYER_SIZE, 0, Math.PI*2); ctx.stroke(); }
    ...
    texts.current.forEach((t, i) => {
      t.y -= 1; t.life -= 0.02; if (t.life > 0) { ctx.font = "bold 16px monospace"; ctx.fillStyle = t.color; ctx.globalAlpha = t.life; ctx.fillText(t.text, t.x - 20, t.y); ctx.globalAlpha = 1.0; } else texts.current.splice(i, 1);
    });
  ...
  const handleStartGame = () => {
      ...
      if (!localStorage.getItem('syncOrSinkTutorial')) { gameStateRef.current = 'TUTORIAL'; setGameState('TUTORIAL'); localStorage.setItem('syncOrSinkTutorial', 'true'); }
      else { gameStateRef.current = 'PLAYING'; setGameState('PLAYING'); initWorld(); }
  }

  const finishTutorial = () => { gameStateRef.current = 'PLAYING'; setGameState('PLAYING'); initWorld(); }
  ...
  return (
    <>
      ...
      {/* LEADERBOARD OVERLAY (Visible on 'Rank' tab) */}
      {activeTab === 'Rank' && (
          <div className="absolute inset-0 bg-black/95 z-30 flex flex-col items-center pt-20 px-4 animate-fade-in">
              <h2 className="text-3xl font-bold text-yellow-400 mb-6 italic">LEADERBOARD</h2>
                  ...
                  {leaderboard.map((entry, i) => (
                      <div key={i} className="flex justify-between items-center bg-white/5 p-3 rounded border border-white/10">
                          <div className="flex items-center gap-3">
                              <span className={`font-bold ${i===0?'text-yellow-400':i===1?'text-gray-300':i===2?'text-orange-400':'text-gray-600'}`}>#{i+1}</span>
                              <span className="text-sm font-bold text-white">{entry.name}</span>
                          </div>
                          <span className="font-mono text-cyan-400">{entry.score}m</span>
                      </div>
                  ))}
              </div>
          </div>
            ...
            {gameState === 'TUTORIAL' && (
               <div className="flex flex-col items-center text-center max-w-[300px] pointer-events-auto">
                   ...
                   <div className="space-y-4 text-sm text-gray-300 mb-8">
                       ...
                       <p>⚠️ Avoid <strong className="text-red-400">BLOCKS</strong>.</p>
                       <p>🟣 <strong className="text-purple-400">GHOST ORB</strong> = Invincible (8s).</p>
                       <p>🛡️ <strong className="text-white">POWERUPS</strong> only work when GROUNDED!</p>
                   </div>
                    ...
                    {!hasUploaded ? (
                        <div className="mt-4 pt-4 border-t border-white/10">
                            <input 
                                ...
                                maxLength={10}
                                className="w-full bg-black/50 border border-white/20 rounded px-3 py-2 text-center text-white text-sm focus:border-cyan-400 outline-none mb-2"
                                value={playerName}
                                onChange={(e) => setPlayerName(e.target.value.toUpperCase())}
                            />
                            <button 
                                onClick={saveToLeaderboard}
                                disabled={!playerName || isUploading}
                                className="w-full bg-green-600 hover:bg-green-500 text-white text-xs font-bold py-2 rounded transition-colors flex justify-center items-center gap-2"
                            >
                                {isUploading ? 'UPLOADING...' : 'UPLOAD SCORE (DATA ON)'}
                            </button>
                        </div>
                    ) : (
                        <div className="mt-4 pt-4 border-t border-white/10 text-green-400 text-xs font-bold">
